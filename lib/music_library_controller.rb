...
class MusicLibraryController
  ...
  def list_artists
    Artist.all.sort{|x, y| x.name <=> y.name}.each.with_index(1) do |artist, index| puts "#{index}. #{artist.name}"
     end
  end

  def list_genres
    Genre.all.sort{|x, y| x.name <=> y.name}.each.with_index(1) do |genre, index| puts "#{index}. #{genre.name}"
    end
  end
